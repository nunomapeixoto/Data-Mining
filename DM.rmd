...
```{r distribution, echo=FALSE, message=FALSE, warning=FALSE, dpi=100, comment=""}
attach(mtcars)
par(mfrow=c(1,2))
...
The metric evaluation to be considered for these forecasts is the **MAE** - Mean Absolute Error. We use a cross validation method with 2 repetitions of 5 folds.
...
With this module the error obtained was with the parameters:
...
The best MAE value obtained with this modules was obtained with the parameters:
