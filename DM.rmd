...
We checked out from the graph that there are some cases (`r round(nrow(new_train_data[new_train_data$TA<new_train_data$TotalBurntArea,c("TA","TotalBurntArea")])/nrow(new_train_data)*100,2)` %) where the burnt area is greater than the total area. This means that in these cases the land area was completely burned repeatedly over time.

```{r times_burnt, echo=FALSE}
tmp<-new_train_data[new_train_data$TA<new_train_data$TotalBurntArea,c("TA","TotalBurntArea")]
...
most_burnt <- new_train_data[round(tmp$TotalBurntArea/tmp$TA)>9, top_names]
